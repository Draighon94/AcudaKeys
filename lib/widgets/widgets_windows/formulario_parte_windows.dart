...
class FormularioParteWindows extends ConsumerStatefulWidget {
  const FormularioParteWindows({super.key});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() {
    return _FormularioParteWindowsState();
  }
}

class _FormularioParteWindowsState
    extends ConsumerState<FormularioParteWindows> {
  ...
  void generarPdf(AppLocalizations l10n) async {
    ...
    //registramos el parte en la firebase
    bool resultado = await ref
        .read(partePDFControllerProvider.notifier)
        .registrarParte(parte);
    if (resultado) {
      //si se ha podido registrar, guardamos el parte en pdf
      await ref
          .read(partePDFControllerProvider.notifier)
          .generarParte(parte)
          .then((ruta) {
            ...
            children: [
              Column(
                children: [
                  SizedBox(
                    width: 500,
                    child: FormDatosVigilante(formKey: formDatosVigilanteKey),
                  ),
                  const SizedBox(
                    width: 500,
                    child: FormResultadosVerificacion(),
                  ),
                  const SizedBox(width: 500, child: FormObservaciones()),
                ],
              ),
              Column(
                children: [
                  SizedBox(
                    width: 500,
                    child: FormSolicitudServicio(
                      formKey: formSolicitudServicioKey,
                    ),
                  ),
                  if (!isGuardando)
                    Padding(
